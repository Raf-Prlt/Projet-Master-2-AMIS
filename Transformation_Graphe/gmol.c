...
void ajouterVoisin(struct Atome *atome, int voisinId) {
    atome->voisins = realloc(atome->voisins, (atome->degre + 1) * sizeof(int));
    atome->voisins[atome->degre] = voisinId;
    atome->degre++;
}*/
...
struct Atome getAtomeById(struct g_mol* molecule, int nb_atomes, int id) {
    ...
    return atomeVide;  // Retourner une structure spéciale pour indiquer l'absence d'atome
}
...
void write_g_mol(struct g_mol* molecule) {
    ...
    // Créer un nom de fichier avec le format "g_mol_(id).dat" pour les données binaires
    char filename[50];
...
struct g_mol read_g_mol(char* filename) {
    ...
    printf("Name: %s , ID: %d\n", test.name, test.Id);
    printf("%d atomes et %d liaisons\n", test.nb_atomes, test.nb_liaisons);
    // Fermer le fichier
    fclose(file);
...
void freeMolecule(struct g_mol *molecule, int nb_atomes) {
    /*for (int i = 0; i < nb_atomes; i++) {
        free(molecule->Atomes[i].voisins);
    }*/
...
void gmol() {
    ...
    DIR *directory;
    struct dirent *entry;
    directory = opendir("../TraitementDonnee/output_molecules/"); // à modifier quand ce sera bon (remplacer test par output_molecules)
    if (!directory) {
    ...
    while ((entry = readdir(directory)) != NULL) {
        ...
        if (strstr(entry->d_name, ".sdf") != NULL) {
            ...
            char filepath[512];
            snprintf(filepath, sizeof(filepath), "../TraitementDonnee/output_molecules/%s", entry->d_name); // à modifier quand ce sera bon
            ...
            while (j < nb_liaisons) {
                ...
                fscanf(file, "\n%3d%3d %d", &liaisons[j].IdA1, &liaisons[j].IdA2, &liaisons[j].Poids);
                if(liaisons[j].IdA1 == 0 || liaisons[j].IdA2 == 0 || liaisons[j].Poids == 0
                    || liaisons[j].IdA1 > nb_atomes || liaisons[j].IdA2 > nb_atomes) {
                    // La lecture n'a pas réussi ou les indices ne sont pas valides, on revient au début de la ligne
                    fseek(file, position_before, SEEK_SET);
                    fscanf(file, "\n %2d%3d %d", &liaisons[j].IdA1, &liaisons[j].IdA2, &liaisons[j].Poids);
                }
                fgets(buffer, sizeof(buffer), file);
            ...
            for (int i = 0; i < nb_atomes; i++) {
                ...
                //molecule->Atomes[i].voisins = NULL;
            }
            ...
            */
            ...
            if ((molecule->Id != -1) && (molecule->name) && (nb_atomes) && (nb_liaisons)) {
            ...
            freeMolecule(molecule, nb_atomes);
    ...
    closedir(directory);
    printf("%d fichiers ont été enregistrés sur les %d initiaux\n", nb_saved, nb_files);
}